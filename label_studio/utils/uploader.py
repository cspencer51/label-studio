...
def tasks_from_file(filename, file, project):
    ...
    try:
        if filename.endswith('.csv') and not is_time_series_only(project):
            ...
            # Python 3.5 compatibility fix https://docs.python.org/3/whatsnew/3.6.html#json
            try:
                tasks = [{'data': {settings.UPLOAD_DATA_UNDEFINED_NAME: json.loads(line)}} for line in lines]
            except TypeError:
                tasks = [{'data': {settings.UPLOAD_DATA_UNDEFINED_NAME: json.loads(line.decode('utf-8'))}} for line in lines]
